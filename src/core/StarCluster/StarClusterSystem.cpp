...
void StarClusterSystem::OnInspect(const std::shared_ptr<EditorLayer>& editorLayer)
{
...
void StarClusterSystem::CalculateStarPositionSync()
{
    ...
    scene->ForEach<StarOrbitProportion, StarPosition, StarOrbit, StarOrbitOffset>(
        ...
        false);
    const auto usedTime = Times::Now() - m_calcPositionTimer;
...
{
    auto scene = GetScene();
    m_applyPositionTimer = Times::Now();
    scene->ForEach<StarPosition, GlobalTransform, Transform, SurfaceColor, DisplayColor>(
        ...
        false);
    m_applyPositionTimer = Times::Now() - m_applyPositionTimer;
}
...
void StarClusterSystem::CopyPosition(const bool &reverse)
{
    ...
    auto matrices = check ? scene->GetOrSetPrivateComponent<Particles>(m_rendererFront.Get()).lock()->m_particleInfoList.Get<ParticleInfoList>()
                          : scene->GetOrSetPrivateComponent<Particles>(m_rendererBack.Get()).lock()->m_particleInfoList.Get<ParticleInfoList>();
    const auto starAmount = scene->GetEntityAmount(m_starQuery);
    matrices->m_particleInfos.resize(starAmount);
    scene->ForEach<GlobalTransform, DisplayColor>(
        Jobs::Workers(),
        m_starQuery,
        [&](int i, Entity entity, GlobalTransform &globalTransform, DisplayColor &displayColor) {
            matrices->m_particleInfos[i].m_instanceMatrix.m_value = globalTransform.m_value;
...
void StarClusterSystem::Start()
{
    ...
    if (m_rendererFront.Get().GetIndex() == 0 && m_rendererBack.Get().GetIndex() == 0)
    {
        ...
        imr->m_mesh = Resources::GetResource<Mesh>("PRIMITIVE_CUBE");
