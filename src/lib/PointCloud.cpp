...
void PointCloud::ApplyCompressed()
{
	...
	{
		particleInfos[i].m_instanceMatrix.m_value = glm::translate((glm::vec3)(compressed[i] + m_offset)) * glm::scale(glm::vec3(m_pointSize));
		particleInfos[i].m_instanceColor = m_colors[i];
	}
...
void PointCloud::ApplyOriginal()
{
	...
	for (int i = 0; i < particleInfos.size(); i++)
	{
		particleInfos[i].m_instanceMatrix.m_value = glm::translate((glm::vec3)(m_points[i] + m_offset)) * glm::scale(glm::vec3(m_pointSize));
		particleInfos[i].m_instanceColor = m_colors[i];
	}
	particleInfoList->SetParticleInfos(particleInfos);
}
