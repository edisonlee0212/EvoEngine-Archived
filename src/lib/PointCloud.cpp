...
void PointCloud::ApplyCompressed()
{
    ...
    particleInfoList->m_particleInfos.resize(compressed.size());
    for (int i = 0; i < particleInfoList->m_particleInfos.size(); i++)
    {
...
void PointCloud::ApplyOriginal()
{
    ...
    particles->m_particleInfoList = ProjectManager::CreateTemporaryAsset<ParticleInfoList>();
    auto particleInfoList = particles->m_particleInfoList.Get<ParticleInfoList>();
    particleInfoList->m_particleInfos.resize(m_points.size());
    for (int i = 0; i < particleInfoList->m_particleInfos.size(); i++)
    {
