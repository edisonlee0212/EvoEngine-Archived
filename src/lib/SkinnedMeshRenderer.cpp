...
void SkinnedMeshRenderer::RenderBound(const std::shared_ptr<EditorLayer>& editorLayer, glm::vec4& color) {
  ...
  glm::vec3 size = m_skinnedMesh.Get<SkinnedMesh>()->bound_.Size() * 2.0f;
  if (size.x < 0.01f)
    size.x = 0.01f;
  if (size.z < 0.01f)
    size.z = 0.01f;
  ...
  editorLayer->DrawGizmoMesh(
      ...
      transform * (glm::translate(m_skinnedMesh.Get<SkinnedMesh>()->bound_.Center()) * glm::scale(size)), 1,
      gizmoSettings);
}
...
void SkinnedMeshRenderer::UpdateBoneMatrices() {
  ...
  if (m_ragDoll) {
    ...
    m_boneMatrices->value.resize(skinnedMesh->bone_animator_indices.size());
    for (int i = 0; i < m_boundEntities.size(); i++) {
      auto entity = m_boundEntities[i].Get();
      if (entity.GetIndex() != 0) {
        m_ragDollTransformChain[i] =
            scene->GetDataComponent<GlobalTransform>(entity).value * animator->offset_matrices_[i];
      }
    }
    for (int i = 0; i < skinnedMesh->bone_animator_indices.size(); i++) {
      m_boneMatrices->value[i] = m_ragDollTransformChain[skinnedMesh->bone_animator_indices[i]];
    }
  } else {
    auto skinnedMesh = m_skinnedMesh.Get<SkinnedMesh>();
    if (animator->bone_size_ == 0)
      return;
    m_boneMatrices->value.resize(skinnedMesh->bone_animator_indices.size());
    for (int i = 0; i < skinnedMesh->bone_animator_indices.size(); i++) {
      m_boneMatrices->value[i] = animator->transform_chain_[skinnedMesh->bone_animator_indices[i]];
    }
  }
}
...
bool SkinnedMeshRenderer::OnInspect(const std::shared_ptr<EditorLayer>& editorLayer) {
  bool changed = false;
  if (editorLayer->DragAndDropButton<Animator>(m_animator, "Animator"))
    changed = true;
  if (ImGui::Checkbox("Cast shadow##SkinnedMeshRenderer", &m_castShadow))
    changed = true;
  if (editorLayer->DragAndDropButton<Material>(m_material, "Material"))
    changed = true;
  if (editorLayer->DragAndDropButton<SkinnedMesh>(m_skinnedMesh, "Skinned Mesh"))
    changed = true;
  ...
  if (const auto animator = m_animator.Get<Animator>()) {
    ...
    if (animator && debugRenderBones) {
      ...
      if (!m_ragDoll) {
        debugRenderingMatrices.resize(animator->transform_chain_.size());
        Jobs::RunParallelFor(animator->transform_chain_.size(), [&](unsigned i) {
          debugRenderingMatrices.at(i).instance_matrix.value = animator->transform_chain_.at(i);
          debugRenderingMatrices.at(i).instance_color = debugRenderBonesColor;
        });
        ...
        Jobs::RunParallelFor(m_ragDollTransformChain.size(), [&](unsigned i) {
          debugRenderingMatrices.at(i).instance_matrix.value = m_ragDollTransformChain.at(i);
          debugRenderingMatrices.at(i).instance_color = debugRenderBonesColor;
        });
      }
      for (int index = 0; index < debugRenderingMatrices.size(); index++) {
        debugRenderingMatrices[index].instance_matrix.value = debugRenderingMatrices[index].instance_matrix.value *
                                                               glm::inverse(animator->offset_matrices_[index]) *
      ...
      editorLayer->DrawGizmoMeshInstancedColored(Resources::GetResource<Mesh>("PRIMITIVE_SPHERE"), particleInfoList,
                                                 ltw.value, debugRenderBonesSize);
    }
    ...
    if (m_ragDoll) {
      ...
      if (ImGui::TreeNode("RagDoll")) {
        for (int i = 0; i < m_boundEntities.size(); i++) {
          if (editorLayer->DragAndDropButton(m_boundEntities[i], "Bone: " + animator->names_[i])) {
            auto entity = m_boundEntities[i].Get();
...
void SkinnedMeshRenderer::Serialize(YAML::Emitter& out) const {
  ...
  if (!m_ragDollTransformChain.empty()) {
    out << YAML::Key << "m_ragDollTransformChain" << YAML::Value
        << YAML::Binary(reinterpret_cast<const unsigned char*>(m_ragDollTransformChain.data()),
...
void SkinnedMeshRenderer::SetRagDoll(bool value) {
  ...
  if (m_ragDoll) {
    const auto scene = GetScene();
    // Resize entities
    m_boundEntities.resize(animator->transform_chain_.size());
    // Copy current transform chain
    m_ragDollTransformChain = animator->transform_chain_;
  ...
  if (const auto scene = GetScene(); scene->IsEntityValid(entity)) {
    if (const auto animator = m_animator.Get<Animator>()) {
      if (resetTransform) {
        ...
        globalTransform.value = m_ragDollTransformChain[index] * glm::inverse(animator->offset_matrices_[index]);
