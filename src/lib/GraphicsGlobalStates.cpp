...
void GraphicsGlobalStates::BindGraphicsPipeline(VkCommandBuffer commandBuffer,
	const std::shared_ptr<GraphicsPipeline>& graphicsPipeline)
{
	assert(graphicsPipeline->m_targetRenderPass->GetVkRenderPass() == m_renderPassApplied->GetVkRenderPass());
	assert(m_subpassIndexApplied == graphicsPipeline->m_targetSubpassIndex);
	vkCmdBindPipeline(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, graphicsPipeline->m_vkGraphicsPipeline);
	m_graphicsPipeline = graphicsPipeline;
}

void GraphicsGlobalStates::BeginRenderPass(VkCommandBuffer commandBuffer, 
	const std::shared_ptr<RenderPass>& targetRenderPass, const std::shared_ptr<Framebuffer>& targetFramebuffer)
{
	assert(!m_renderPassApplied);
	m_renderPassApplied = targetRenderPass;
	m_subpassIndexApplied = 0;
	VkRenderPassBeginInfo renderPassBeginInfo{};
	renderPassBeginInfo.sType = VK_STRUCTURE_TYPE_RENDER_PASS_BEGIN_INFO;
	renderPassBeginInfo.renderPass = targetRenderPass->GetVkRenderPass();
	renderPassBeginInfo.framebuffer = targetFramebuffer->GetVkFrameBuffer();
	renderPassBeginInfo.renderArea.offset = { 0, 0 };
	renderPassBeginInfo.renderArea.extent = { targetFramebuffer->GetWidth(), targetFramebuffer->GetHeight() };

	std::vector<VkClearValue> clearValues{};
	for(const auto& i : targetFramebuffer->GetAttachments())
	{
		clearValues.emplace_back(i->RefClearValue());
	}
...
void GraphicsGlobalStates::NextSubpass(VkCommandBuffer commandBuffer, VkSubpassContents subpassContents)
{
	vkCmdNextSubpass(commandBuffer, subpassContents);
	m_subpassIndexApplied++;
}

void GraphicsGlobalStates::EndRenderPass(VkCommandBuffer commandBuffer)
{
	assert(m_renderPassApplied);
	vkCmdEndRenderPass(commandBuffer);
	m_renderPassApplied.reset();
