...
uint32_t Cubemap::GetTextureStorageIndex() const {
  return texture_storage_handle_->m_value;
}
...
#pragma region Depth
  ...
  depth_image_info.extent.width = storage.m_image->GetExtent().width;
  depth_image_info.extent.height = storage.m_image->GetExtent().height;
...
#pragma endregion
  ...
  Graphics::ImmediateSubmit([&](VkCommandBuffer command_buffer) {
    storage.m_image->TransitImageLayout(command_buffer, VK_IMAGE_LAYOUT_ATTACHMENT_OPTIMAL);
#pragma region Viewport and scissor
    VkRect2D render_area;
    render_area.offset = {0, 0};
    render_area.extent.width = storage.m_image->GetExtent().width;
    render_area.extent.height = storage.m_image->GetExtent().height;
    VkViewport viewport;
    viewport.x = 0.0f;
    viewport.y = 0.0f;
    ...
    equirectangular_to_cubemap->m_states.m_viewPort = viewport;
    equirectangular_to_cubemap->m_states.m_scissor = scissor;
#pragma endregion
    for (int i = 0; i < 6; i++) {
#pragma region Lighting pass
      ...
      equirectangular_to_cubemap->m_states.m_cullMode = VK_CULL_MODE_NONE;
      equirectangular_to_cubemap->m_states.m_colorBlendAttachmentStates.clear();
      equirectangular_to_cubemap->m_states.m_colorBlendAttachmentStates.resize(1);
      for (auto& i : equirectangular_to_cubemap->m_states.m_colorBlendAttachmentStates) {
        i.colorWriteMask =
      ...
      mesh->DrawIndexed(command_buffer, equirectangular_to_cubemap->m_states, 1);
      vkCmdEndRendering(command_buffer);
#pragma endregion

      Graphics::EverythingBarrier(command_buffer);
    }
    storage.m_image->TransitImageLayout(command_buffer, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
  });
}
bool Cubemap::OnInspect(const std::shared_ptr<EditorLayer>& editor_layer) {
  bool changed = false;
  if (const auto& storage = RefStorage(); !storage.m_imTextureIds.empty()) {
    ...
    for (int i = 0; i < 6; i++) {
      ImGui::Image(
          storage.m_imTextureIds[i],
          ImVec2(storage.m_image->GetExtent().width * debug_scale, storage.m_image->GetExtent().height * debug_scale),
