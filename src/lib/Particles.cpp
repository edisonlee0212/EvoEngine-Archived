...
void Particles::RecalculateBoundingBox() {
  ...
  for (const auto& i : particleInfoList->PeekParticleInfoList()) {
    const glm::vec3 center = i.instance_matrix.value * glm::vec4(meshBound.Center(), 1.0f);
    const glm::vec3 size = glm::vec4(meshBound.Size(), 0) * i.instance_matrix.value / 2.0f;
    minBound = glm::vec3((glm::min)(minBound.x, center.x - size.x), (glm::min)(minBound.y, center.y - size.y),
                         (glm::min)(minBound.z, center.z - size.z));

    maxBound = glm::vec3((glm::max)(maxBound.x, center.x + size.x), (glm::max)(maxBound.y, center.y + size.y),
                         (glm::max)(maxBound.z, center.z + size.z));
  }
  m_boundingBox.max = maxBound;
  m_boundingBox.min = minBound;
}

bool Particles::OnInspect(const std::shared_ptr<EditorLayer>& editorLayer) {
  bool changed = false;
  if (ImGui::Checkbox("Cast shadow##Particles", &m_castShadow))
    changed = true;
  if (editorLayer->DragAndDropButton<Material>(m_material, "Material"))
    changed = true;
  if (editorLayer->DragAndDropButton<Mesh>(m_mesh, "Mesh"))
    changed = true;
  if (editorLayer->DragAndDropButton<ParticleInfoList>(m_particleInfoList, "ParticleInfoList"))
    changed = true;

  if (const auto particleInfoList = m_particleInfoList.Get<ParticleInfoList>()) {
    ImGui::Text(
        ("Instance count##Particles" + std::to_string(particleInfoList->PeekParticleInfoList().size())).c_str());
